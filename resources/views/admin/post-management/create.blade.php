            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-xl font-bold text-slate-900">Viết Bài Mới</h2>
                            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Blog & Content Management
                            </p>
                        </div>
                    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if ($errors->any())
                <div class="mb-6 flex items-center p-4 text-red-800 border-t-4 border-red-500 bg-red-50 rounded-lg shadow-sm"
                    ...
                    <div class="ml-3">
                        <h3 class="text-sm font-medium">Có lỗi xảy ra khi lưu bài viết:</h3>
            ...
            <form id="main-post-form" action="{{ route('posts.store') }}" method="POST">
                @csrf
                    ...
                    <div class="lg:col-span-8 space-y-8">
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                {{-- Slug --}}
                                <div>
                                    <label class="block text-sm font-bold text-slate-800 mb-2">Slug (Đường dẫn) <span
                                            class="text-red-500">*</span></label>
                                ...
                                <div>
                                    ...
                                    <textarea name="excerpt" rows="3" placeholder="Tóm tắt nội dung bài viết..."
                                        class="w-full rounded-xl border border-gray-300 bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all px-4 py-3 text-sm text-slate-700">{{ old('excerpt') }}</textarea>
                                </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select name="status"
                                        ...
                                        <option value="published" @selected(old('status') === 'published' || !old('status'))>
                                            Xuất bản ngay</option>
                                    </select>
                                </div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <label for="is_featured" class="text-sm font-medium text-slate-700">Bài viết nổi
                                        bật</label>
                                </div>
                                ...
                                <button type="submit"
                                    class="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-md shadow-blue-200">
                                    Lưu bài viết
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            ...
                            <div class="space-y-6">
                                    ...
                                    <select name="post_category_id"
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}"
                                                @selected(old('post_category_id') == $category->id)>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <div
                                        class="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-2 border border-slate-100 rounded-lg bg-slate-50/50">
                                        @foreach($tags as $tag)
                                    ...
                                    @if($tags->isEmpty())
                                        <p class="text-xs text-slate-400 italic">Chưa có tag nào. <a
                                                href="{{ route('post-tags.index') }}" class="text-blue-500 hover:underline">Tạo
                                                mới</a></p>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            ...
                            <div class="space-y-4">
                                <div id="media-preview"
                                    class="relative group aspect-video rounded-xl bg-slate-100 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden cursor-pointer"
                                    onclick="openMediaModal()">
                                    <div class="text-center group-hover:scale-110 transition-transform duration-200"
                                        id="media-placeholder">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-slate-300 mx-auto"
                                    ...
                                    <button type="button" id="remove-img"
                                        class="hidden absolute top-2 right-2 p-1 bg-white/90 rounded-full text-red-500 shadow-sm hover:bg-white transition-all">
                                ...
                                <p class="text-xs text-slate-400 text-center italic">Đề xuất tỉ lệ 16:9 (Ví dụ: 1280x720)
                                </p>
                            </div>
                        </div>
                    </div>
