...
@section('content')
    ...
    <div class="min-h-screen bg-[#f8fafc] pb-12">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-xl font-bold text-slate-900">Cập nhật Danh Mục</h2>
                            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">ID: #{{ $category->id }}
                                -
                                Category Management System</p>
                        </div>
                    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form id="main-category-form" action="{{ route('categories.update', $category->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                <div>
                                    ...
                                    <textarea id="description" name="description" rows="4" placeholder="Nhập mô tả danh mục"
                                        class="w-full rounded-xl border border-gray-300 bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all px-4 py-3 text-sm text-slate-700">{{ old('description', $category->description) }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="p-4 rounded-xl border border-slate-100 bg-slate-50">
                                    ...
                                    <div class="space-y-2 text-xs text-slate-600">
                                        <p class="flex justify-between">
                                            <span>Ngày tạo:</span>
                                            <span
                                                class="font-medium">{{ $category->created_at->format('d/m/Y H:i') }}</span>
                                        </p>
                                        <p class="flex justify-between">
                                            <span>Cập nhật cuối:</span>
                                            <span
                                                class="font-medium">{{ $category->updated_at->format('d/m/Y H:i') }}</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
